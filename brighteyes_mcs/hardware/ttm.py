...
class TtmRemoteManager(object):
    def __init__(self, ip, port, local_executable):
        logger.debug("TtmRemoteManager.__init__")
    ...
    def connect(self):
        ...
        logger.debug("%s %s", self.parameter.HOST, self.parameter.PORT)
    ...
    def parse(self, data):
        ...
        if "READY!" in data:
            ...
            logger.debug("%s %s", "self.parameter.last_filename = ", data[5:-1])
    ...
    def close(self):
        logger.debug("self.parameter.sock.close()")
    ...
    def wait_end_of_file_loop(self, handle, timeout=3):
        logger.debug("wait_end_of_file_loop")
        t = time.time()
        while (time.time() - t) < timeout:
            if self.parameter.last_filename != "":
                logger.debug("%s %s %s", "wait_end_of_file_thread", self.parameter.last_filename, t)
