...
class H5Manager:
    ...
    def init_dataset(self, dataset_name, shape, timebinsPerPixel, channels, dtype):
        ...
        logger.debug("%s %s", dataset_name, self.h5dset[dataset_name].shape)
    ...
    def _add_to_dataset(self, dataset_name, buffer_for_save, current_rep, current_z):
        ...
        # debug("_add_to_dataset", dataset_name, dims)
        self.h5dset[dataset_name].resize(dims)
        # debug(self.h5dset[dataset_name].shape)
        self.h5dset[dataset_name][current_rep, current_z, :] = buffer_for_save
        # debug()
        # debug(self.h5dset[dataset_name].shape, buffer_for_save.shape)

    def close(self):
        self.update_threads_number()
        logger.debug("h5 closed")
        self.h5file.close()
    ...
    def metadata_add_dict(self, group_name, mydict=None, *, legacy_name_map=None):
        ...
        logger.debug("%s %s", "Metadata h5 added at %s: " % group_name, meta_data_h5_debug)
    def metadata_add_initial(self, comment):
        logger.debug("metadata_add_initial")
        self.h5file.attrs["default"] = "data"
        self.h5file.attrs["data_format_version"] = DATA_FORMAT_VERSION
        self.h5file.attrs["comment"] = comment

    def metadata_add_thumbnail(self, image):
        logger.debug("metadata_add_thumbnail")
        try:
            ...
            logger.debug("Thumbnail creation ERROR")
            logger.debug(ex)
    ...
    def run(self):
        ...
        while keep_running:
            ...
            try:
                ...
                logger.debug("%s %s", "H5ManagerProcess error", repr(ex))
                if wait:
        ...
        if h5mgr is not None:
            try:
                ...
                logger.debug("%s %s", "H5ManagerProcess close error", repr(ex))
