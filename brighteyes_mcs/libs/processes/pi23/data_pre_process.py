...
class Pi23DataPreProcess(mp.Process):
    ...
    def run(self):
        print_debug("Pi23DataPreProcess RUN - PID:", os.getpid())
        while not self.stop_event.is_set():
            ...
            for fifo_name, payload in dict_from_queue.items():
                ...
                try:
                    ...
                    print_debug("Pi23DataPreProcess decode error", repr(exc))
                    continue
                if decoded.size == 0:
                    continue
                print_debug(
                    "Pi23DataPreProcess decoded",
                    fifo_name,
                    "samples",
                    raw_bunch.sample_count,
                    "words",
                    decoded.size,
                )
                pi23_debug(
    ...
    def stop(self):
        print_debug("Pi23DataPreProcess STOP")
        self.stop_event.set()
        self.terminate()
