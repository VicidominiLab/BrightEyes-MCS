...
from PySide6.QtCore import (
    ...
    QBuffer,
    QObject)
from ..application.gui_gateway import GuiApplicationGateway
...
class FastAPIServerThread(threading.Thread):
    ...
    def __init__(self, main_window, host="127.0.0.1", port=8000):
        ...
        self.gateway = (
            main_window
            if isinstance(main_window, GuiApplicationGateway)
            else GuiApplicationGateway(main_window)
        )
        self.server = None
        command_catalog = {
            "preview": {
        ...
        self.signal.start.connect(self.gateway.start_acquisition)
        self.signal.stop.connect(self.gateway.stop)
        self.signal.preview.connect(self.gateway.start_preview)
        ...
        async def read_full_state():
            ...
            """
            return self.gateway.full_state()
        ...
        async def read_gui():
            ...
            """
            mydict = self.gateway.configuration()
        ...
        async def update_item(request: Request):
            ...
            try:
                ...
                if len(mydict_processed.keys()) > 0:
                    ...
                    self.gateway.update_configuration(mydict_processed)
                    return {"status": "OK"}
                else:
                    return {"status": "BAD"}
        ...
        async def send_array():
            ...
            try:
                ...
                exporter = exporters.ImageExporter(self.gateway.preview_image_item())
        ...
        async def send_array():
            ...
            try:
                ...
                exporter = exporters.ImageExporter(self.gateway.fingerprint_image_item())
        ...
        async def send_array():
            ...
            try:
                # Create a sample NumPy array
                np_array = self.gateway.preview_array()
        ...
        async def send_array():
            ...
            try:
                # Create a sample NumPy array
                np_array = self.gateway.fingerprint_array()
                # Convert the array to bytes
                array_bytes = np_array.tobytes()
                shape_str = ','.join(map(str, np_array.shape))
                dtype_str = str(np_array.dtype)
