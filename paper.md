...
The sample positioning and the scanning are controlled by the analog outputs connected to the galvo mirrors, or a piezo stage. The analog outputs supported are 8 and the user can select for each channel to be used as X,  Y,  Z, or a constant voltage. The possibility of setting a constant voltage is useful for other types of elements such as Acousto-optic modulators (AOM).
